...
#  1.2: MS updates: change to 'basefs' for label, minor updates to customise for my env.
#
...
overlayDev=$(blkid -L "overlayfs")

fail(){
  echo -e "$1"
  if [ -n "${overlayDev}" ]; then
    if [ -d "/mnt" ]; then
      mkdir -p /mnt/errorout
      mount -t f2fs ${overlayDev} /mnt/errorout
      if [ $? -eq 0 ]; then
        echo -e "$1" >> /mnt/errorout/booterror.txt
      else
        #try mount boot partition
        mount /dev/mmcblk1p1 /mnt/errorout
        echo -e "$1" >> /mnt/errorout/booterror.txt
      fi
    fi
  fi
...
rootMountOpt="defaults,noatime"
rootFsType="ext4"
echo "Try if a filesystem with label 'basefs' is avaialble"
rootDev=$(blkid -L "basefs")
if [ $? -gt 0 ]; then
